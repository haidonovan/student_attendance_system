...
export default function CheckAttendancePage() {
  ...
  // all class name (it's already working)
  useEffect(() => {
    try {
  ...
  // auto fetch teacher subject (it's already working)
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    const fetchStandbyClasses = async () => {
      try {
        const response = await fetch("/api/standby-classes")
        if (!response.ok) throw new Error("Failed to fetch classes")
        const data = await response.json()
        setStandbyClasses(data[0])
      } catch (err) {
  ...
  const fetchStudents = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `/api/dashboard/teacher/attendance?standByClassId=${selectedClass}&sectionId=${selectedSession}&page=${page}`,
      )
  ...
  return (
    <SidebarProvider>
      <SidebarInset>
        <pre>{classNames.length > 0 ? classNames.map((c, i) => (<pre key={i}>{c.name}</pre>)) : "No Data Yet!!"}</pre>
                  ...
                  <Select value={selectedClass} onValueChange={setSelectedClass}>
                    ...
                    <SelectContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
                      {classNames.map((cls, index) => (
                        <SelectItem key={cls} value={index} className="hover:bg-gray-100 dark:hover:bg-gray-700">
                          {cls.name}
                        </SelectItem>
                      ))}
                    </SelectContent>
